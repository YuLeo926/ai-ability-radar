...
不测量 IQ，也不推断服务端发生了什么。快速和标准批次只展示证据；完整批次可以在严格匹配的
本地历史基线上显示“稳定”“观察”或“证据不足”，但“很可能退化”仍保持关闭，等待真实校准和独立复核。
...
同一批次只能包含同一种轨道：客户端与 CLI 会分成两个不可直接比较的证据组。应用会冻结题包、
评分规则、模型路由、推理档位、运行入口和费用上限，再以平衡顺序执行重复测试。结果矩阵的每个格子
都显示样本数并可展开证据；请求模型、客户端可见模型和提供商默认路由会分别标注，不会混成一个名称。

快速批次用于先试用流程；标准批次增加 CLI 重复次数；完整批次才冻结历史基线并进行统计比较。
完整的匹配条件、样本门槛、信号含义、导出字段和费用上限见[批量检测方法](docs/batch-scan-methodology.md)。
...
- 应用在开始前显示任务启动数、最大 provider turns、预计/最大时长和授权失效时间；token 或订阅配额
  无法可靠换算时会明确显示“未知”，不会伪造费用估算。
- 配额暂停后的恢复需要新的明确授权，并复用原运行记录；应用不会静默重试或把暂停前后当成两个样本。
...
更完整的行为与安全边界见[方法说明](docs/methodology.md)、[批量检测方法](docs/batch-scan-methodology.md)、
[隐私说明](docs/privacy.md)、[安全说明](docs/security.md)和[故障排查](docs/troubleshooting.md)。
