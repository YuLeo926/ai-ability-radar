...
CLI 模型字段为空白（输入留空）表示使用该 CLI 的默认路由，历史中规范化存为 `default`。只有
Codex CLI 与 Claude Code 把字面 `default` 显示为默认路由；ChatGPT 客户端与 Claude 客户端中的
同一字面值仍是用户填写的模型标签。显式提供模型时，模型名会传给所选 CLI；选择推理档位时也会
显式传递。模型、推理档位与 quick 运行模式都是历史键的一部分，不能跨键拼接趋势。
...
启动/恢复和公开 JSON/HTML 报告使用同一规则。旧历史若含无效标签，前端
只显示稳定的“模型名称不可显示”占位符，不修改原始存储记录；公开报告
创建与验证则失败关闭，不把该无效原文写入 JSON 或 HTML。
...
未知但可显示的旧自定义推理档位仍按原文显示。若旧记录包含 `Cc`、`Cf`、
`Default_Ignorable_Code_Point` 或双向格式字符，存储值不被改写，所有前端
历史、恢复和结果界面统一显示“推理档位不可显示”；公开 JSON/HTML 报告
创建与验证失败关闭。
...
classic EOCD 必须位于 EOF 前最后 22 字节、archive comment length 必须为
0，且 EOCD 必须恰好结束于 EOF；随后验证 central directory、Windows 路径
安全、规范化碰撞及精确文件成员集合，全部通过后才允许提取和发布。
