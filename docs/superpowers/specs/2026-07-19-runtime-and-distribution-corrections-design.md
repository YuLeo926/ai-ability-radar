...
- 对“报告模型”应用与手动自定义档位相同的 `Cc`、`Cf`、
  `Default_Ignorable_Code_Point` 和双向格式字符拒绝策略，并保持去除
  首尾空白后的 1–120 字符限制；前端启动、后端启动/恢复和公开报告规则
  一致；旧历史的无效标签在前端仅显示安全占位符、不改写存储，公开报告
  创建失败关闭；`default` 只对 Codex CLI 与 Claude Code 表示默认路由，
  在两个客户端目标中保留为字面模型名；
- 以 `schemas/reasoning-effort-display.json` 作为 4 个目标 × 8 个规范值
  的共享显示策略，允许明确的目标覆盖；同一目标/值在设置、恢复、历史、
  结果和 HTML 中显示一致，规范存储值不变；
- 在压缩前按应用相同的 registry、manifest、引用文件和内容哈希不变量校验源题包与暂存副本，并在压缩前再次校验暂存副本；
- 使用复用 `PackRegistry::parse` 与 `PackLoader::load` 的第一方 Rust
  helper 在 source、staged、pre-compression 和 extracted 四个检查点
  保证运行时 JSON 解析一致；保留现有 Node 精确结构与内容封印检查，
  helper 只作为构建工具，不进入便携包；
- 解压前解析原始 ZIP central directory，拒绝不支持的编码/结构、Windows
  不安全路径与规范化别名，并要求原始文件成员集合与暂存 payload 完全一致；
  classic EOCD 必须是 EOF 前最后 22 字节、archive comment length 为 0，
  且记录恰好结束于 EOF；
- 拒绝手动自定义档位中的 Unicode `Cc`、`Cf`、默认可忽略字符和双向格式控制字符，去除首尾空白后必须仍有 1–40 个可见字符；
- 旧记录中的不可显示自定义档位不改写存储，前端统一显示
  `推理档位不可显示`，公开 JSON/HTML 报告失败关闭；未知但可显示的值
  继续原样显示；
