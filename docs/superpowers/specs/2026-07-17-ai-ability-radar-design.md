...
v0.2.1 的“报告模型”是受控显示文本：去除首尾空白后须为 1–120
个字符，并拒绝 Unicode `Cc`、`Cf`、`Default_Ignorable_Code_Point`
与双向格式控制字符。前端启动、后端启动/恢复和公开报告使用同一规则；
旧历史中的无效标签只显示稳定的本地化占位符，不改写原记录。推理档位
标签由一份 4 个目标 × 8 个规范值的共享策略定义；允许目标覆盖（例如
ChatGPT `low` 显示“轻度”），但设置、恢复、历史、结果和 HTML 对同一
目标/值必须一致，持久化值仍保持原规范字符串。
...
- 至少积累 3 次可比检测后建立个人基线。
- 默认基线使用最早 3 次有效、可比检测的中位数，当前检测永远不参与自己的基线计算。
- 基线建立后不会被新结果静默改写；用户可以显式选择一次已知良好的检测或重新建立基线。
- 重新建立基线会创建新的趋势段，旧基线和旧结论继续保留。
- 正常波动使用基线样本的中位绝对偏差估计，降低单次异常影响。
...
- 基线波动量 `V` 定义为基线样本中位绝对偏差的 `1.4826` 倍。
- 疑似下降阈值 `T` 定义为 `max(5 分, 1.5 × V)`。
- 明显下降阈值 `C` 定义为 `max(10 分, 2 × V)`。
- 正常波动：下降不超过 `T`。
- 疑似下降：下降超过 `T`，但尚未满足明显下降条件。
- 明显下降：下降超过 `C`，并在重复测试或至少两个分类中再次出现。
- 高可信下降：配对变化的 95% 区间上界仍低于 0，或连续两次深度检测出现明显下降。
- 样本不足：展示当前结果，不生成降智结论。
...
- 用户取消。
- 应用异常或电脑重启。
- 基础设施超时，例如 CLI 无响应或网络调用中断。
- 在没有基础设施异常时耗尽固定代理时间或轮数预算。
- 验证器异常。
- 模型完成但答案或测试未通过。

额度、网络、取消、应用异常、基础设施超时和验证器异常标记为无效样本或运行稳定性信息。在相同固定预算下，代理未能按时完成任务属于可重复的任务结果，计入能力失败；模型完成但答案或测试未通过也计入能力失败。
...
v0.2 保持单目标体检。模型名称、推理档位、来源和确认状态必须先可靠
落库，作为后续多目标扫描不会混错模型的基础。
...
- 个人基线、配对比较和可信度。
- 深度检测、题目重复和变体。
- 多模型、多推理档位的同类目标批次扫描；客户端和 CLI 因题包与执行器
  不同，先分成不可直接比较的组展示。
- 快速对照只展示描述性差异；只有足够的重复样本和兼容历史基线才能
  给出版本化的回归信号。
- 真实仓库轻量题与 DeepSWE 兼容入口。
- 容器或 WSL 隔离。

进入 v0.5 前必须先完成 v0.2 的真实用户试运行和指标校准规格。
批量扫描的目标指纹、会话隔离、额度确认、调度、基线兼容和分阶段
交付以
...
- GitHub CLI 一键发布。
- Windows 客户端自动化实验适配器。
- 剪贴板和自动化能力由用户主动开启，只在当前检测流程中生效。
- 自动化适配器不得访问登录输入框或读取身份凭据。
- 自动化失败时始终可退回手动流程。
...
- 本机定时检测和更多模型。
- 第三方插件权限声明与签名。
- 定时检测必须由用户主动开启，并设置时间和消耗上限。
- 多平台改造不得降低 Windows 已有功能或评分可追溯性。
